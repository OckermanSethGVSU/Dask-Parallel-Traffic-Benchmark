...
def main(runid):
    ...
    my_train(x_train=x_train, mean=mean, std=std, y_train=y_train, ycl_train=ycl_train, x_val=x_val, y_val=y_val, backend='gloo')
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if (epoch - 1) % args.print_every == 0:
            if worker_rank == 0:
                ...
                print(f"Epoch {epoch} VAL |  time: {t2 - t1} loss: {mvalid_loss}, mape: {mvalid_mape}, rmse: {mvalid_rmse}",flush=True)
